...
class TenantService
{
    ...
    public function getTenantUsers(Tenant $tenant, string $status = 'active'): Collection
    {
        return $tenant->users()
            ...
            ->map(function($user) {
                ...
                $rawPermissions = $user->pivot->permissions ? 
                    json_decode($user->pivot->permissions, true) : [];
                
                // Set processed permissions (granted only)
                if (isset($rawPermissions['granted']) && is_array($rawPermissions['granted'])) {
                    $user->custom_permissions = $rawPermissions['granted'];
                } else {
                    $user->custom_permissions = [];
                }
                
                // Also set raw permissions for frontend display
                $user->raw_custom_permissions = $rawPermissions;
    ...
    private function ensureUniqueSlug(string $slug): string
    {
        ...
        while (Tenant::where('slug', $slug)->exists()) {
            ...
            // Prevent infinite loop (shouldn't happen, but safety)
            if ($counter > 1000) {
